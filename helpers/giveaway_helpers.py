...
async def parse_commands_arguments(context, bot, type: CommandTypes, args):
    ...
    elif(type == CommandTypes.REDEEMABLE):
        return await parse_arguments_redeemable(context, bot, args)


async def parse_redeemable_link(context, redeemable_url):
    get_request = requests.get(redeemable_url + ".json")
    redeemable_data_json = get_request.json()

    if 'claimerAddress' in redeemable_data_json["slot"]["status"]:
        await context.message.author.send('URL looks valid, but the NFT is already redeemed.')
        return

    if not 'isActive' in redeemable_data_json["slot"]["status"]:
        await context.message.author.send('URL looks valid, but there is no redeemable at this link.')
        return

    if redeemable_data_json["nft"]["contractAddress"] != "0xa115dfbb5ab7adffad2f7f25fa7a5c227616c2c3" and redeemable_data_json["nft"]["contractAddress"] != "0x4F41d10F7E67fD16bDe916b4A6DC3Dd101C57394":
        await context.message.author.send("URL looks valid, but you can't give away anything else than a Flowetpach FLOWER")
        return

    if redeemable_data_json["chainId"] != 137 and redeemable_data_json["chainId"] != 1:
        await context.message.author.send("URL looks valid, but you can only give FLOWERs on Ethereum or Polygon network")
        return

    if redeemable_data_json["chainId"] == 1:
        flower_network = "eth"
    elif redeemable_data_json["chainId"] == 137:
        flower_network = "poly"
    flower_identifier = flower_network + \
        "-" + redeemable_data_json["nft"]["id"]

    return flower_identifier
...
async def send_new_giveaway_embed(context, author, flower_identifier, flower_rarity, flower_image_url, reaction, end_time, tweet_url):
    ...
    embed.add_field(
        name='FLOWER', value=f"[{flower_identifier}](https://flowerpatch.app/card/{flower_identifier})", inline=False)
    ...
    if len(tweet_url) > 0:
        embed.add_field(name='Twitter link',
                        value=f"[click me]({tweet_url})", inline=False)
    else:
        embed.add_field(name='Twitter link',
                        value="*No link yet...*", inline=False)
...
async def send_giveaway_reroll_embed(context, next_winner, author, participants_count, message_id):
    ...
    embed.add_field(name=':drum: New winner is... :drum: ',
                    value=f"{winner_obj.mention}!", inline=False)
    embed.set_footer(
        text=f'{participants_count} users were participating in this giveaway')
    result = await context.send(embed=embed)
...
async def send_new_list_embed(context, ongoing_giveaways):
    ...
    embed.set_footer(
        text=f"Time to create this message: {round((end - start).total_seconds(), 2)}s.")
...
async def get_giveaway_embed(context, giveaway):
    ...
    if config["environment"] != "Dev":
        filtered_list = [
            x for x in users_list if not determine_bot_or_author(x, giveaway["author"])]
    else:
...
async def send_successfully_aborted_embed(context, giveaway_url):
    embed = discord.Embed()
    embed.add_field(name=f"A giveaway has been aborted...",
                    value=f"I successfully aborted [the giveaway]({giveaway_url}) and removed the tweet about it.", inline=False)
    await context.send(embed=embed)
    return
...
def format_general_stats(giveaways):
    ...
    for giveaway in giveaways:
        ...
        giveaways_result["total_rarity"] = giveaways_result["total_rarity"] + \
            giveaway["flower_rarity"]
        if giveaways_result["winners"].get(giveaway["winner"]) and len(giveaway["rerolls"]) == 0:
            giveaways_result["winners"][giveaway["winner"]
        ...
        if len(giveaway["rerolls"]) > 0:
            ...
            if giveaways_result["winners"].get(giveaway["rerolls"][-1]["winner"]):
                giveaways_result["winners"][giveaway["rerolls"][-1]["winner"]
                                            ] = giveaways_result["winners"][giveaway["rerolls"][-1]["winner"]] + 1
            else:
                giveaways_result["winners"][giveaway["rerolls"]
                                            [-1]["winner"]] = 1
        if giveaway["status"] == "ABORTED":
            giveaways_result["count_aborted"] += 1

    sorted_winners = dict(sorted(
        giveaways_result["winners"].items(), key=lambda item: item[1], reverse=True))
    sorted_authors = dict(sorted(
        giveaways_result["authors"].items(), key=lambda item: item[1], reverse=True))
    sorted_emojis = dict(sorted(
        giveaways_result["emojis"].items(), key=lambda item: item[1], reverse=True))
...
def format_user_stats(giveaways, user_id):
    ...
    stats_result["emojis"] = dict(
        sorted(stats_result["emojis"].items(), key=lambda item: item[1], reverse=True))
...
async def print_user_stats_results(context, stats_results, user):
    ...
    embed.add_field(name=f"{user.display_name} has started **{stats_results['giving_count']}** giveaways",
                    value=f"That is a total of **{stats_results['giving_rarity_count']} rarity** given away!\n{emojis_text}", inline=False)
    embed.add_field(name=f"{user.display_name} has participated in **{stats_results['participation_count']}** giveaways",
                    value=f"Winning **{stats_results['winning_count']}** of them (**{'{0:.2f}'.format((stats_results['winning_count'] / stats_results['participation_count']) * 100)}%** won)!\nFor a total of **{stats_results['winning_rarity_count']} rarity** won", inline=False)
...
async def add_twitter_link_to_embed(context, message_id, tweet_url):
    ...
    old_embed.embeds[0].set_field_at(
        4, name="Twitter link", value=f"{tweet_url}", inline=False)
...
async def update_original_message_when_aborted(context, message_id):
    old_embed = await context.fetch_message(message_id)
    old_embed.embeds[0].set_field_at(
        3, name="Ends in", value=f"*Aborted*", inline=False)
    old_embed.embeds[0].set_field_at(
        4, name="Twitter link", value=f"*Removed*", inline=False)
    await old_embed.edit(embed=old_embed.embeds[0])
    return
...
async def update_original_message_when_ended(context, message_id, winner):
    ...
    old_embed.embeds[0].add_field(
        name="Winner", value=f"{winner_obj.mention}", inline=False)
    await old_embed.edit(embed=old_embed.embeds[0])
    return
...
async def update_original_message_when_rerolled(context, message_id, next_winner):
    ...
    winner_obj = await context.bot.fetch_user(next_winner)
    old_embed.embeds[0].set_field_at(
        3, name="Ends in", value=f"*Rerolled*", inline=False)
    old_embed.embeds[0].set_field_at(
        5, name="Winner", value=f"{winner_obj.mention}", inline=False)
    await old_embed.edit(embed=old_embed.embeds[0])
    return
...
async def update_time_left_on_list_message(context, message_id, ongoing_giveaways):
    ...
    if old_embed != 0:
        if len(old_embed.embeds[0].fields) != len(ongoing_giveaways):
            ...
            for f in results:
                old_embed.embeds[0].set_field_at(
                    field_iterator, name=f"{f[0]}", value=f"{f[1]}", inline=False)
                field_iterator += 1
        await old_embed.edit(embed=old_embed.embeds[0])
...
async def react_to_message(context, message_id, reaction):
    if config["environment"] == "Dev":
        ...
        giveaway_channel = context.bot.get_channel(713882535964442745)
    embedded_message = await giveaway_channel.fetch_message(message_id)
...
async def parse_arguments_create(context, context_emojis, args):
    ...
    # Start getting image
    parsed_args["flower_url"] = get_flower_image_url(
        parsed_args["flower_identifier"])
    if parsed_args["flower_url"] is None:
        ...
        if not save_flower_png(parsed_args["flower_url"], parsed_args["flower_identifier"], False):
            await error_invalid_flower_url_image(context)
            return None
    parsed_args["flower_data_url"] = get_flower_data_url(
        parsed_args["flower_identifier"])
    if parsed_args["flower_data_url"] is None:
        await error_invalid_flower_identifier(context)
        return None
    else:
        parsed_args["flower_rarity"] = get_flower_rarity(
            parsed_args["flower_data_url"])
...
async def parse_arguments_redeemable(context, context_emojis, args):
    splitted_args = args.split(" ")
    # Arg[0] = !giveaway
    #Arg[1] = redeemable
    # Arg[2] = time delta
    ...
    # Arg[2] get time delta
    parsed_args["end_time"] = get_end_time(splitted_args[2].lower())
    if parsed_args["end_time"] is None:
        await error_invalid_end_time(context)
        return None

    # Arg[3] get emoji
    parsed_args["reaction"] = splitted_args[3]
    if not giveaway_emojis_helpers.check_emoji_valid(context_emojis, parsed_args["reaction"]):
        await error_invalid_emoji(context)
        return None
    return parsed_args
